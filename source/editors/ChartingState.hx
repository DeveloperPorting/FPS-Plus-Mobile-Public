...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addNoteUI():Void{
		...
		for(k => v in NoteType.typeGroups){
			if(!noteTypeGroupsArray.contains(k)){
				noteTypeGroupsArray.push(k);
			}
		}
		noteTypeGroupsDrop = new FlxUIDropDownMenuScrollable(140, 50, FlxUIDropDownMenu.makeStrIdLabelArray(noteTypeGroupsArray, true), function(group:String){
			setUpNoteTypeDrop(noteTypeGroupsArray[Std.parseInt(group)]);
		});
		noteTypeGroupsDrop.selectedLabel = "All Note Types";

		tab_group_note.add(noteTagText);
		tab_group_note.add(noteType);
		tab_group_note.add(noteTypeGroupsDrop);

		setUpNoteTypeDrop("All Note Types", false);

		UI_box.addGroup(tab_group_note);
	}

	function setUpNoteTypeDrop(group:String, remove:Bool = true):Void{
		if(remove){ tab_group_note.remove(noteTypeDrop); }
		noteTypesArray = [""];
		noteTypesArray = noteTypesArray.concat(NoteType.typeGroups.get(group));
		noteTypeDrop = new FlxUIDropDownMenuScrollable(10, 50, FlxUIDropDownMenu.makeStrIdLabelArray(noteTypesArray, true), function(type:String){
	...
	override function update(elapsed:Float){
		...
		if (!anyTextHasFocus){
			...
			if ((FlxG.keys.justPressed.R || virtualPad.buttonX.pressed)){
			...
			if ((!FlxG.keys.pressed.SHIFT || !virtualPad.buttonY.pressed)){
				if ((FlxG.keys.pressed.W || FlxG.keys.pressed.S || FlxG.keys.pressed.UP || FlxG.keys.pressed.DOWN) || (virtualPad.buttonUp.pressed || virtualPad.buttonDown.pressed)){
					FlxG.sound.music.pause();
					vocals.pause();
					vocalsOther.pause();
					...
					if ((FlxG.keys.pressed.W || FlxG.keys.pressed.UP) || (virtualPad.buttonUp.pressed) && strumLine.y > gridBG.y){
			...
			if (FlxG.mouse.x > gridBG.x
				...
				if ((FlxG.keys.pressed.CONTROL || virtualPad.buttonC.pressed) && (FlxG.keys.pressed.SHIFT || virtualPad.buttonY.pressed)){
					dummyArrow.y = Math.floor(FlxG.mouse.y / (GRID_SIZE / 4)) * (GRID_SIZE / 4);
				}
				else if ((FlxG.keys.pressed.SHIFT || virtualPad.buttonY.pressed) && (FlxG.keys.pressed.ALT || virtualPad.buttonZ.pressed)){
					dummyArrow.y = Math.floor(FlxG.mouse.y / (GRID_SIZE * 4/6)) * (GRID_SIZE * 4/6);
				}
				else if ((FlxG.keys.pressed.ALT || virtualPad.buttonZ.pressed)){
					dummyArrow.y = Math.floor(FlxG.mouse.y / (GRID_SIZE * 4/3)) * (GRID_SIZE * 4/3);
				}
				else if ((FlxG.keys.pressed.CONTROL || virtualPad.buttonC.pressed)){
			...
			if (FlxG.keys.justPressed.ENTER || virtualPad.buttonA.justPressed){
				...
				if((FlxG.keys.pressed.CONTROL || virtualPad.buttonC.pressed) && curSection > 0){
			...
			if ((FlxG.keys.pressed.SHIFT || virtualPad.buttonY.pressed))
				shiftThing = 4;
			if ((FlxG.keys.justPressed.RIGHT || virtualPad.buttonRight.justPressed) || FlxG.keys.justPressed.D)
				changeSection(curSection + shiftThing);
			if ((FlxG.keys.justPressed.LEFT || virtualPad.buttonLeft.justPressed) || FlxG.keys.justPressed.A)
				changeSection(curSection - shiftThing);
		}
...
// || FlxG.keys.justPressed.X  || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V
			...
			curRenderedNotes.forEach(function(x:Note) {
				...
				if(x.y < strumLine.y && !x.playedEditorClick && FlxG.sound.music.playing){
					if(x.editorBFNote){
						...
						lilBf.animation.play("" + (x.direction % 4), true);
					}
					else if(!x.editorBFNote){
						if(opClick.checked){ FlxG.sound.play(Paths.sound("tick"), 0.6); }
						lilOpp.animation.play("" + (x.direction % 4), true);
					}
				}
	...
	function updateHeads(?changedCharacters:Bool = false):Void{
		...
		if(changedCharacters){
			...
			leftIcon.centerOrigin();
			rightIcon.centerOrigin();
	...
	private function addNotesToRender(curSec:Int, ?secOffset:Int = 0){
		...
		for (i in section){
			...
			note.y = (getYfromStrum((Math.max(Conductor.stepCrochet * _song.notes[curSection].lengthInSteps * secOffset, daStrumTime - sectionStartTime())) % (Conductor.stepCrochet * _song.notes[curSection].lengthInSteps)));
	...
	function selectNote(note:Note):Void{
		...
		for(sectionNote in _song.notes[curSection].sectionNotes){
			if(Utils.inRange(sectionNote[0], note.strumTime, 3) && sectionNote[1] == note.absoluteNumber && Utils.inRange(sectionNote[2], note.sustainLength, 3)){
				curSelectedNote = sectionNote;
				if(sectionNote.length >= 4){ noteType.text = sectionNote[3]; }
				else{ noteType.text = ""; }
				break;
			}
		}
	...
	private function addNote(_noteStrum:Float, _direction:Int, ?skipSectionCheck:Bool = false):Void{
		var noteAdjust:Array<Int> = [0, 1, 2, 3, 4, 5, 6, 7];
		...
		var direction = noteAdjust[_direction];
		...
		_song.notes[curSection].sectionNotes.push([noteStrum, direction, noteSus, noteTypeString]);

		curSelectedNote = _song.notes[curSection].sectionNotes[_song.notes[curSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.TAB){
			_song.notes[curSection].sectionNotes.push([noteStrum, (direction + 4) % 8, noteSus, noteTypeString]);
		}
	...
	private function saveLevel(){
		...
		if ((data != null) && (data.length > 0)){
		  #if mobile
		   try {
			File.saveContent("saves/" + _song.song.toLowerCase() + diffDropFinal + ".json", data.trim());
			extension.androidtools.Tools.showAlertDialog("Level Saved!", "Successfully saved the " + _song.song.toLowerCase() + diffDropFinal + ".json" + " file", {name: "OK", func: null}, null);
		   } catch (e:Dynamic) {
            extension.androidtools.Tools.showAlertDialog("FPS Plus Mobile Error", "The file " + _song.song.toLowerCase() + diffDropFinal + ".json" + " could not be saved. Please check if the saves folder actually exists!", {name: "OK", func: null}, null);
		   }
		  #else
	...
	private function saveEvents(){
		...
		if ((data != null) && (data.length > 0)){
		  #if mobile
		   try {
			File.saveContent("saves/events.json", data.trim());
			extension.androidtools.Tools.showAlertDialog("Events Saved!", "Successfully saved the events.json file", {name: "OK", func: null}, null);
		   } catch (e:Dynamic) {
            extension.androidtools.Tools.showAlertDialog("FPS Plus Mobile Error", "The file events.json could not be saved. Please check if the saves folder actually exists!", {name: "OK", func: null}, null);
		   }
		  #else
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE, onSaveComplete);
	...
	private function addEventNote(_noteStrum:Float, _direction:Int):Void{
		...
		for (i in _events.events){
			if (Utils.inRange(i[1], noteStrum, 3) && (eventTag == i[3] || direction == i[2])){
				return;
			}
		}
		...
		_events.events.push([section, noteStrum, direction, eventTag]);
		_events.events.sort(sortByEventStuff);
		
		//trace("Slot: " + direction);
